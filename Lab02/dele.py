...
def load_and_augment(image_path, mask_path):
    image, mask = load_image_and_mask(image_path, mask_path)
    # concatenate so augmentation applies equally
    concat = tf.concat([image, mask], axis=-1)
    concat = data_augmentation(concat)
    image = concat[..., :3]
    mask = concat[..., 3:]
    return image, mask

# Create augmented dataset
train_aug_dataset = tf.data.Dataset.from_tensor_slices((train_image_paths, train_mask_paths))
train_aug_dataset = train_aug_dataset.map(load_and_augment, num_parallel_calls=tf.data.AUTOTUNE)
train_aug_dataset = train_aug_dataset.shuffle(100).batch(BATCH_SIZE).prefetch(tf.data.AUTOTUNE)
...
# Reuse Week 1 model definition
improved_model = get_unet_model(input_size=(IMG_HEIGHT, IMG_WIDTH, 3))

improved_model.compile(
    optimizer=tf.keras.optimizers.Adam(learning_rate=1e-4),
    loss=lambda y_true, y_pred: 0.7*tf.keras.losses.binary_crossentropy(y_true, y_pred) + 0.3*dice_loss(y_true, y_pred),
    metrics=["accuracy"]
)
